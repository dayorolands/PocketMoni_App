...
public class DataTopUpModel {
    private static final List<DataTopUpModel.Plan> planDetails = new ArrayList<>();
    private static final String DKEY = "datatopup_cat_key";
    public final String PIN_KEY = "A8BF4C78F2EEEDB96FAF3A0655454345";
    public final String CUSTOM_PAN = "0000000000000000";

    public DataTopUpModel(){ }

    private static String billsName, billsCode, description, image, pin, amount, customerId, paymentRef, customerName;
    Activity activity;
    public DataTopUpModel(Activity activity, DataTopUpModel.ICategoryResponse internetCallbacks){
        this.activity = activity;
        getBillsCategories(internetCallbacks);
    }

    String category,categoryImage;
    public DataTopUpModel(String category, String categoryImage){
    ...
    public void setBillsName(String billsName) {
        DataTopUpModel.billsName = billsName;
    }
    ...
    public void setBillsCode(String billsCode) {
        DataTopUpModel.billsCode = billsCode;
    }
    ...
    public void setDescription(String description) { DataTopUpModel.description = description; }

    public String getImage() { return image; }

    public void setImage(String image) {
        DataTopUpModel.image = image;
    }
    ...
    public void setCustomerId(String customerId) {
        DataTopUpModel.customerId = customerId;
    }
    ...
    public void setPaymentRef(String paymentRef) {
        DataTopUpModel.paymentRef = paymentRef;
    }

    public String getCustomerName() { return customerName; }

    public void setCustomerName(String customerName) { DataTopUpModel.customerName = customerName; }

    public List<DataTopUpModel.Plan> getCategories(){
        return planDetails;
    }

    public DataTopUpModel.Plan getPlanDetails(String category){
        for(DataTopUpModel.Plan plan : planDetails){
            if(plan.getBillsName().toLowerCase().equals(category.toLowerCase())){
                return plan;
            }
        }
        return new DataTopUpModel.Plan("", "", "");
    }
    ...
    private void getBillsCategories(DataTopUpModel.ICategoryResponse internetCallback){
        this.internetListener = internetCallback;
        new Thread(()->{
            try{
                String json = SharedPref.get(activity,DKEY,"");
                ...
                for(int i=0; i<catBillsName.size(); i++) {
                    planDetails.add(new DataTopUpModel.Plan(
                            catBillsName.get(i),
                            catBillsCode.get(i),
                            catImage.get(i)
                    ));
...
//                }
    ...
    private DataTopUpModel.ICategoryResponse internetListener;
